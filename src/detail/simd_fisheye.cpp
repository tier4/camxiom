...
)
{
  ...
  for (int i = 0; i < simd_count; i += 4)
  {
    ...
    for (int j = 0; j < 4; ++j)
    {
      ...
      else
      {
        ...
        const PixelResult scalar_result = fisheye::rayToPixel(model, ray);
  ...
  for (int i = simd_count; i < count; ++i)
  {
    const int off = i * 3;
    const Eigen::Vector3f ray(rays_xyz[off], rays_xyz[off + 1], rays_xyz[off + 2]);
    const PixelResult result = fisheye::rayToPixel(model, ray);
...
)
{
  if (model.distortion.has_tilt)
  {
    ...
    for (int i = 0; i < count; ++i)
    {
      const int off = i * 3;
      const Eigen::Vector3f ray(rays_xyz[off], rays_xyz[off + 1], rays_xyz[off + 2]);
      const PixelResult result = omnidirectional::rayToPixel(model, ray);
      u_out[i] = result.pixel.u;
      v_out[i] = result.pixel.v;
      if (statuses_out != nullptr)
      {
  ...
  for (int i = 0; i < simd_count; i += 4)
  {
    ...
    for (int j = 0; j < 4; ++j)
    {
      ...
      {
        // Full scalar fallback (see rayToPixelBatchFisheyeSse).
        const int off = index * 3;
        const Eigen::Vector3f ray(rays_xyz[off], rays_xyz[off + 1], rays_xyz[off + 2]);
        const PixelResult scalar_result = omnidirectional::rayToPixel(model, ray);
        u_out[index] = scalar_result.pixel.u;
        v_out[index] = scalar_result.pixel.v;
        if (statuses_out != nullptr)
        {
  ...
  for (int i = 0; i < avx_count; i += 8)
  {
    ...
    for (int j = 0; j < 8; ++j)
    {
      ...
      else
      {
        const int off = index * 3;
        const Eigen::Vector3f ray(rays_xyz[off], rays_xyz[off + 1], rays_xyz[off + 2]);
        const PixelResult r = omnidirectional::rayToPixel(model, ray);
